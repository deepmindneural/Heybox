    ...
    const fetchOrders = async () => {
      ...
      try {
        ...
        if (isLocalDevelopment && token) {
          ...
          if (session) {
            // Obtenemos los pedidos del usuario desde SQLite
            const userOrders = dbService.getOrdersByUserId(session.user_id);
            
            // Convertimos al formato esperado por el componente
            const formattedOrders = userOrders.map(order => ({
              _id: order.id?.toString() || '',
              numeroPedido: order.id?.toString().padStart(6, '0') || '',
              restaurante: {
                _id: order.restaurant_id.toString(),
                nombre: order.restaurant_name || 'Restaurante',
                logo: order.restaurant_logo
              },
              total: order.total,
              estado: order.status,
              fechaCreacion: order.created_at || new Date().toISOString(),
              items: 3 // Valor por defecto, idealmente se contaría de los items reales
            }));
            
            setOrders(formattedOrders);
          }
        } else if (token) {
          // Usar la API remota (código existente)
