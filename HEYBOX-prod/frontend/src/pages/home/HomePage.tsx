...
const HomePage: React.FC = () => {
  ...
  useEffect(() => {
    const loadNew = async () => {
      try {
        ...
        console.error('Error al cargar restaurantes nuevos:', error);
        setError('Error al cargar restaurantes nuevos');
      } finally {
        setLoadingNew(false);
      }
    };
  ...
  useEffect(() => {
    if (isAuthenticated) {
      cargarCreditosUsuario().catch(error => {
  ...
  useEffect(() => {
    setLoading(loadingPromos || loadingFeatured || loadingNew || loadingCategories);
  }, [loadingPromos, loadingFeatured, loadingNew, loadingCategories]);
