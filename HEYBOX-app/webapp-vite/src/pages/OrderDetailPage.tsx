  ...
  useEffect(() => {
    const fetchOrderDetails = async () => {
      ...
      try {
        ...
        // Verificar si estamos en desarrollo local o usando la API remota
        ...
        if (isLocalDevelopment && token) {
          ...
          // Validamos la sesión para obtener el ID del usuario
          const session = dbService.validateSession(token);
          
          if (session) {
            // Obtenemos los detalles del pedido desde SQLite
            const orderDetails = dbService.getOrderById(parseInt(orderId));
            
            if (orderDetails) {
              // Convertir al formato esperado por el componente
              const formattedOrder: OrderDetail = {
                _id: orderDetails.id?.toString() || '',
                numeroPedido: orderDetails.id?.toString().padStart(6, '0') || '',
                usuario: {
                  _id: orderDetails.user_id.toString(),
                  nombre: session.name || 'Usuario',
                  telefono: session.telefono || '+57 301 234 5678'
                },
                restaurante: {
                  _id: orderDetails.restaurant_id.toString(),
                  nombre: orderDetails.restaurant_name || 'Restaurante',
                  direccion: {
                    calle: orderDetails.restaurant_address || 'Calle Principal',
                    ciudad: 'Bogotá',
                    codigoPostal: '110111',
                    ubicacion: {
                      lat: 4.624335,
                  ...
                  logo: orderDetails.restaurant_logo || 'https://via.placeholder.com/300x200?text=Restaurant'
                },
                productos: (orderDetails.items || []).map(item => ({
                  producto: {
                    _id: item.menu_item_id.toString(),
                    nombre: item.name || 'Producto',
                    precio: item.price,
                    imagen: item.image || 'https://via.placeholder.com/300x200?text=Food'
                  },
                ...
                metodoPago: {
                  tipo: orderDetails.payment_method === 'tarjeta' ? 'tarjeta' : 'efectivo',
                  detalles: {
                    ultimosDigitos: orderDetails.payment_method === 'tarjeta' ? '1234' : undefined,
                    transaccionId: orderDetails.payment_method === 'tarjeta' ? 'TX-' + Date.now() : undefined
                  }
                },
                ...
                estadoHistorial: [
                  {
                    estado: orderDetails.status,
                    fecha: orderDetails.created_at || new Date().toISOString(),
                    comentario: 'Pedido procesado correctamente'
                  }
                ],
                codigoVerificacion: Math.random().toString().substring(2, 8),
                subtotal: orderDetails.total,
                total: orderDetails.total,
                fechaCreacion: orderDetails.created_at || new Date().toISOString(),
                fechaActualizacion: orderDetails.updated_at || new Date().toISOString()
              };
        ...
        const mockOrder: OrderDetail = {
          _id: orderId || '1',
          numeroPedido: orderId?.padStart(6, '0') || '000001',
          usuario: {
            _id: '1',
          ...
          estadoHistorial: [
            ...
            },
            {
              ...
              comentario: 'Pedido confirmado por el restaurante'
            },
            {
              estado: 'preparando',
    ...
    fetchOrderDetails();
  }, [API_URL, orderId, token, isAuthenticated]);
