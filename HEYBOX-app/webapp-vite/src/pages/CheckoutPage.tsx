...
const CheckoutPage: React.FC = () => {
  ...
  // Flag para desarrollo local (usar true para SQLite, false para API remota)
  ...
  useEffect(() => {
    if (isAuthenticated && usuario) {
      ...
      const metodosUsuario = (usuario as Usuario).metodosPago || [];
      
      // Convertir métodos de pago del usuario al formato requerido
      const formasPagoUsuario: FormaPago[] = metodosUsuario.map((metodo: MetodoPago, index: number) => ({
        id: index.toString(),
        tipo: metodo.tipo,
      ...
      const usuarioTyped = usuario as Usuario;
      if (usuarioTyped.direccion) {
        setDireccionEntrega({
          calle: usuarioTyped.direccion.calle || '',
          ciudad: usuarioTyped.direccion.ciudad || '',
          codigoPostal: usuarioTyped.direccion.codigoPostal || '',
          referencia: ''
        });
      }
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Usar almacenamiento local o API remota según configuración
      if (useLocalStorage && token && restaurante) {
        // Validar sesión para obtener el ID del usuario
        const session = dbService.validateSession(token);
        
        if (session) {
          // Crear el pedido en SQLite
          const orderItems = items.map(item => ({
            menuItemId: parseInt(item.id),
            quantity: item.cantidad,
            price: item.precio,
            notes: item.opciones?.join(', ') || ''
          }));
          
          // Determinar el método de pago
          let paymentMethod = 'efectivo';
          if (formaPagoSeleccionada === 'nueva-tarjeta' || formaPagoSeleccionada.startsWith('tarjeta-')) {
            paymentMethod = 'tarjeta';
          }
          
          // Crear la dirección formateada
          const formattedAddress = `${direccionEntrega.calle}, ${direccionEntrega.ciudad}, ${direccionEntrega.codigoPostal}`;
          
          // Crear el pedido
          const newOrder = dbService.createOrder({
            userId: session.userId,
            restaurantId: parseInt(restaurante.id),
            total: totalPrice,
            address: formattedAddress,
            paymentMethod: paymentMethod,
            notes: direccionEntrega.referencia || '',
            items: orderItems,
            status: 'pendiente'
          });
          ...
          if (newOrder && newOrder.id) {
            ...
            navigate(`/pedidos/${newOrder.id}`, {
              state: {
                success: true,
                numeroPedido: newOrder.id.toString(),
        ...
        // Cuando se implementa la API remota, aquí iría el código para enviar los datos al servidor
        ...
        setTimeout(() => {
          ...
          const respuestaPedido = {
            ...
            numeroPedido: Math.floor(Math.random() * 100000).toString().padStart(6, '0'),
            codigoVerificacion: Math.random().toString().substring(2, 8),
            estado: 'pendiente',
          ...
          // Limpiar carrito y redirigir
          clearCart();
          navigate(`/pedidos/${respuestaPedido.id}`, {
            state: {
              success: true,
              numeroPedido: respuestaPedido.numeroPedido,
              codigoVerificacion: respuestaPedido.codigoVerificacion
            }
          });
        }, 2000);
      }
